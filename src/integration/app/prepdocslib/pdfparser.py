...
class DocumentAnalysisParser(Parser):
    ...
    async def parse(self, content: IO) -> AsyncGenerator[Page, None]:
        ...
        async with DocumentIntelligenceClient(endpoint=self.endpoint, credential=self.credential) as document_intelligence_client:
            ...
            document_intelligence_results = await poller.result()
            offset = 0
